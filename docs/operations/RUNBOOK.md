...
- **Speech Evidence:** `audio_word_timeline_v1.json` records evidence status, not an objective claim about the source. Non-empty accepted ASR segments are `detected`; zero accepted and zero discarded segments are `not_detected` and may continue with zero words/narration; zero accepted but one or more guard-discarded segments are `uncertain` and fail closed. Empty-set exact-once validations never imply that speech existed, and OCR/captions must not be promoted to narration.
